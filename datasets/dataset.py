...
class ImageDataTrain(data.Dataset):
    def __init__(self,root):
        self.root=root
        self.images_dir = os.path.join(self.root, 'image', 'train')
        self.targets_dir = os.path.join(self.root, 'gtfine','train')
        self.edge_dir = os.path.join(self.root, 'gtfine', 'train')
        self.images = []
        self.targets = []
        self.edge = []
        # 对应读取图片和标签
        for road in os.listdir(self.images_dir):
            ...
            target_name = '{}{}'.format(road.split('.jpg')[0], '.png')
    ...
    def __getitem__(self, item):
        sal_image = load_image(self.images[item])
        sal_label = load_sal_label(self.targets[item])
        sal_edge = load_edge_label(self.edge[item])
...
class ImageDataTest(data.Dataset):
    def __init__(self,root):
        self.root = root
        self.images_dir = os.path.join(self.root, 'image', 'val')
        self.targets_dir = os.path.join(self.root, 'gtfine', 'val')
        self.images = []
        self.targets = []
        # 对应读取图片和标签
        for road in os.listdir(self.images_dir):
    ...
    def __getitem__(self, item):
        image = load_image(self.images[item])
        label = load_sal_label(self.targets[item])
        image = torch.Tensor(image)
        label = torch.Tensor(label)


        #
        # image = load_image(os.path.join(self.image_root, self.image_list[item]))
        # image = torch.Tensor(image)
        sample = {'val_image': image, 'val_label': label}
        return sample
...
def get_loader(root,batch_size, mode='train', num_thread=1):
    shuffle = False
    if mode == 'train':
        shuffle = True
        dataset = ImageDataTrain(root)
    else:
        dataset = ImageDataTest(root)
