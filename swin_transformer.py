...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # build layers
        self.layers0= BasicLayer(dim=int(embed_dim * 2 ** 0),
        ...
        self.layers1= BasicLayer(dim=int(embed_dim * 2 ** 1),
                               ...
                               use_checkpoint=use_checkpoint)
        self.layers2= BasicLayer(dim=int(embed_dim * 2 ** 2),
                               ...
                               use_checkpoint=use_checkpoint)
        self.layers3= BasicLayer(dim=int(embed_dim * 2 ** 3),
    ...
    def forward_features(self, x):
        ...
        x0 = self.norm0(x0).transpose(1, 2)  # B c L
        x0 = x0.view(B, 192, 28, 28)
        x1= self.norm1(x1).transpose(1, 2)  # B c L
        x1 = x1.view(B, 384, 14, 14)
        x2 = self.norm2(x2).transpose(1, 2)  # B c L
        x2 = x2.view(B, 768, 7, 7)
        x3 = self.norm3(x3).transpose(1, 2)  # B c L
        x3 = x3.view(B, 768, 7, 7)
        # x = self.norm(x)  # B L C
...
#     if pretrained:
#         pretrained_dict = torch.load('/home/wyy/PycharmProjects/ABS/exper/aeg_c414/DFR-source/swin_tiny_patch4_window7_224.pth')
