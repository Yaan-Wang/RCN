...
def visulization(img_file, mask_path, score_map_path, saving_path):
    ...
    # image
    img_name1 = '{}{}'.format(img_file.split('.bmp')[0], '.jpg')
    image_file1= os.path.join('/home/wyy/PycharmProjects/ABS1xiugai/absegdata/gapsab/image/test/', img_name1)
    img = cv2.imread(image_file1, cv2.IMREAD_COLOR)
    img = cv2.resize(img, (224, 224))


    # binary score {0, 255}
    score_file = os.path.join(score_map_path, img_file)
    score = cv2.imread(score_file, cv2.IMREAD_GRAYSCALE)
    img = img[:, :, ::-1]  # bgr to rgb
    img[..., 1] = np.where(score == 255, 255, img[..., 1])

    # save
    imsave(os.path.join(saving_path, "{}".format(img_name1)), img)
...
def visulization_score(img_file, mask_path, score_map_path, saving_path):
    ...
    superimposed_img = heatmap * 0.7 + superimposed_img * 0.8

    # save
    cv2.imwrite(os.path.join(saving_path, "{}".format(img_name1)), superimposed_img)
    imsave(os.path.join(saving_path, "gt_{}".format(img_name1)), img)
